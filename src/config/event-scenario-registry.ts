...
export const EVENT_SCENARIO_REGISTRY: EventScenarioMeta[] = [
  ...
  },
  {
    ...
    trigger_type: 'CUSTOM_EVENT',
    fire_description: 'PRODUCT_LIST/SEARCH 페이지에서 제품 리스트가 노출되고 아이템 변수가 선언되었을 때 ap_search 트리거로 발생. 페이지 로드 자동 발생이 아닌, 제품 존재 조건부 발생',
    testability: 'action',
    scenario_status: 'included',
    page_types: ['PRODUCT_LIST', 'SEARCH'],
    target_selector: 'predictive-search input[type="search"], #Search-In-Modal, input[name="q"]',
    notes: 'auto-fire 아님. ap_search CUSTOM_EVENT 트리거 기반. PRODUCT_LIST에서는 제품 리스트 존재 시 발생, SEARCH에서는 검색 실행 후 결과에 제품이 있을 때 발생',
  },
  {
    ...
    dataLayer_event: 'ap_search',
    trigger_type: 'CUSTOM_EVENT',
    fire_description: '검색 기능 사용 후 검색 결과에 아이템 정보 변수가 선언되어 있으면 ap_search 트리거로 수집. 페이지 로드 자동 발생이 아닌, 검색 액션 후 조건부 발생',
    testability: 'action',
    scenario_status: 'included',
    page_types: ['SEARCH'],
    target_selector: 'predictive-search input[type="search"], #Search-In-Modal, input[name="q"]',
    notes: 'auto-fire 아님. view_item_list과 같은 ap_search 트리거 공유. Content Group 필터로 SEARCH_RESULT 페이지에서만 발생. 검색 액션 필수',
  },
