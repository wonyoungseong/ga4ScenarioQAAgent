...
// 상품 리스트 페이지 URL (브랜드 페이지 - 설화수)
const TEST_URL = 'https://www.amoremall.com/kr/ko/display/brand?brandCd=SULWHASOO';
...
async function main() {
  ...
  try {
    ...
    const screenshotPath = './output/product_list_test.png';
    ...
    console.log('\n========================================');
    console.log('상품 리스트 페이지 예상 이벤트 분석 결과');
    console.log('========================================\n');
    ...
    // select_item 확인
    const selectItemFire = result.actuallyCanFire.find(e => e.eventName === 'select_item');
    const selectItemBlocked = result.gtmBlockedEvents.find(e => e.eventName === 'select_item');

    console.log('\n【select_item 이벤트 상태】');
    if (selectItemFire) {
      console.log('✅ select_item: 발생 가능');
    } else if (selectItemBlocked) {
      console.log(`🚫 select_item: 차단됨 - ${selectItemBlocked.summary}`);
    } else {
      console.log('❓ select_item: 상태 불명');
    }
