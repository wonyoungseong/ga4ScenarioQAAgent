...
| 파라미터 | 추출 위치 | 추출 규칙 | 예시 |
|---------|----------|----------|------|
| `item_name` | 상품명 영역 (대형 텍스트) | 화면에 표시된 전체 텍스트 | "설화수 자음생크림" |
| `price` | 가격 표시 영역 | 숫자만 추출 (통화 기호, 쉼표 제거) | 180000 |
| `item_brand` | 브랜드명 영역 또는 로고 | 텍스트 또는 로고에서 추출 | "설화수" |
| `item_id` | URL 파라미터 | `onlineProdCode` 또는 유사 파라미터 | "111070002290" |
| `item_category` | 브레드크럼 또는 카테고리 영역 | 대분류 > 중분류 형식 | "스킨케어 > 크림" |
...
#### 1) 상품명 (`item_name`)
```
위치: 상품 이미지 옆 또는 아래, 가장 눈에 띄는 텍스트
규칙:
- 화면에 표시된 전체 상품명을 추출
- 브랜드명이 포함된 경우 그대로 추출 (예: "설화수 자음생크림")
- 말줄임('...')이 있는 경우 보이는 텍스트까지만 추출

확신도 기준:
- HIGH: 상품명 영역에 명확히 표시
- MEDIUM: 제목과 부제목이 혼합된 경우
- LOW: 이미지 내 텍스트만 존재
```

#### 2) 가격 (`price`, `value`)
```
위치: 상품명 근처, 통화 기호와 함께 표시
규칙:
- 숫자만 추출 (₩, 원, , 제거)
- 할인가가 있는 경우: 실제 판매가 추출 (취소선 없는 가격)
- 정가: `original_price`로 별도 추출
- 소수점은 정수로 반올림

예시:
- "₩180,000" → 180000
- "180,000원" → 180000
- "25,000 (정가 30,000)" → price: 25000, original_price: 30000

확신도 기준:
- HIGH: 가격이 명확히 표시
- MEDIUM: 옵션별 가격이 다른 경우
- LOW: 가격이 이미지 내에만 존재
```
...
규칙:
- 텍스트로 표시된 경우: 그대로 추출
- 로고로만 표시된 경우: 로고에서 브랜드명 추론

주의사항:
- dataLayer에서 영문명을 사용할 수 있음 (예: "Sulwhasoo" vs "설화수")
- 사이트 패턴 확인 필요

확신도 기준:
- HIGH: 텍스트로 명확히 표시
- MEDIUM: 로고에서 추론
- LOW: 브랜드 표시 없음 (URL이나 컨텍스트에서 추론)
```
...
위치: URL 파라미터, data 속성
규칙:
- URL에서 onlineProdCode, productId 등 파라미터 추출
- URL 예: /product/detail?onlineProdCode=111070002290 → "111070002290"

확신도 기준:
- HIGH: URL에 명확히 포함
- MEDIUM: 숨겨진 data 속성에서 추출
- LOW: 추론 필요
```
...
스크린샷을 분석하여 다음 파라미터 값을 추출하세요:

1. item_name: 상품명 영역에서 추출
2. price: 가격 영역에서 숫자만 추출
3. item_brand: 브랜드명 영역 또는 로고에서 추출
4. item_id: URL에서 상품 코드 추출
5. item_category: 브레드크럼에서 카테고리 추출

출력 형식:
{
  "items": [{
    "item_name": {"value": "...", "confidence": "HIGH/MEDIUM/LOW", "sourceLocation": "추출 위치"},
    "price": {"value": 숫자, "confidence": "HIGH/MEDIUM/LOW", "sourceLocation": "추출 위치"},
    ...
  }]
}
```
...
□ price가 dataLayer의 price와 일치하는지 확인 (숫자 형식)
□ item_brand가 일치하는지 확인 (영문/한글 차이 주의)
□ item_id가 일치하는지 확인
□ 불일치 시 추출 규칙 또는 사이트 특수 패턴 확인
```
